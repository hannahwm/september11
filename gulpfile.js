...
gulp.task('images', function(){
  ...
  .pipe(cache(imagemin()))
  .pipe(gulp.dest('dist/interactive/2021/09/september11/images'))
  .pipe(browserSync.reload({
    stream: true
  }))
...
gulp.task('video', function(){
  return gulp.src('app/video/**/*.+(mp4|webm)')
  .pipe(cache(imagemin()))
  .pipe(gulp.dest('dist/interactive/2021/09/september11/video'))
  .pipe(browserSync.reload({
    stream: true
  }))
  .pipe( notify({ message: "video tasks have been completed!"}) );
});

gulp.task('audio', function(){
  return gulp.src('app/audio/**/*.+(wav|mp3)')
  .pipe(cache(imagemin()))
  .pipe(gulp.dest('dist/interactive/2021/09/september11/audio'))
  .pipe(browserSync.reload({
    stream: true
  }))
  .pipe( notify({ message: "audio tasks have been completed!"}) );
});
...
gulp.task('watch', ['browserSync', 'sass', 'scripts', 'copyJSLibraries', 'images', 'video', 'audio', 'fileinclude', 'html'], function (){
...
gulp.task('default', function (callback) {
  runSequence(['clean:dist', 'sass', 'images', 'video', 'audio', 'fileinclude', 'scripts', 'copyJSLibraries', 'useref', 'html', 'browserSync', 'watch'],
    callback
  )
})
